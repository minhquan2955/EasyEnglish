...
export default function RegistrationManagement() {
  ...
  if (user?.role !== 'admin') {
    return (
      <div className="p-8 text-center text-gray-400">
        Bạn không có quyền truy cập trang này. Chỉ Admin mới có thể quản lý các đăng ký tư vấn.
      </div>
    );
  }
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl text-white tracking-tight font-light">
          Registration Management
        </h1>
        <p className="text-gray-400 mt-1">
          Quản lý danh sách các yêu cầu đăng ký tư vấn học từ trang chủ của hệ thống.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        ...
        <div className="bg-[#1a1b1e] border border-gray-800 rounded-[8px] p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Tổng số</p>
            <h3 className="text-2xl text-white mt-1 font-semibold">{stats.total}</h3>
          </div>
        ...
        <div className="bg-[#1a1b1e] border border-gray-800 rounded-[8px] p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Chờ liên hệ</p>
            <h3 className="text-2xl text-amber-500 mt-1 font-semibold">{stats.pending}</h3>
          </div>
        ...
        <div className="bg-[#1a1b1e] border border-gray-800 rounded-[8px] p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Đã liên hệ</p>
            <h3 className="text-2xl text-sky-500 mt-1 font-semibold">{stats.contacted}</h3>
          </div>
        ...
        <div className="bg-[#1a1b1e] border border-gray-800 rounded-[8px] p-4 flex items-center justify-between">
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Hoàn thành</p>
            <h3 className="text-2xl text-emerald-500 mt-1 font-semibold">{stats.completed}</h3>
          </div>
        ...
        <div className="bg-[#1a1b1e] border border-gray-800 rounded-[8px] p-4 flex items-center justify-between col-span-2 md:col-span-1">
          <div>
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wider">Từ chối</p>
            <h3 className="text-2xl text-rose-500 mt-1 font-semibold">{stats.rejected}</h3>
          </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#1a1b1e] border border-gray-800 p-4 rounded-[8px]">
        ...
        <div className="relative flex-1 max-w-md">
          <MagnifyingGlass size={18} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500" />
          <input
            type="text"
        ...
        <div className="bg-[#1a1b1e] rounded-[8px] border border-gray-800 overflow-hidden shadow-lg">
          <div className="p-4 bg-[#121314] border-b border-gray-800 flex justify-between items-center">
            <h3 className="text-white font-medium text-sm">
              Kết quả hiển thị: <span className="text-ps-blue font-bold">{filteredRegistrations.length}</span> đăng ký
            </h3>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap md:whitespace-normal">
              <thead className="bg-[#121314] text-gray-400 border-b border-gray-800 text-xs uppercase tracking-wider">
                <tr>
                  <th className="px-6 py-4 font-medium w-12 text-center">STT</th>
                  <th className="px-6 py-4 font-medium min-w-[150px]">Phụ huynh</th>
                  <th className="px-6 py-4 font-medium min-w-[120px]">Liên hệ</th>
                  <th className="px-6 py-4 font-medium min-w-[150px]">Học viên (Tuổi)</th>
                  <th className="px-6 py-4 font-medium max-w-[250px] whitespace-normal">Ghi chú yêu cầu</th>
                  <th className="px-6 py-4 font-medium min-w-[120px]">Ngày gửi</th>
                  <th className="px-6 py-4 font-medium min-w-[120px]">Trạng thái</th>
                  <th className="px-6 py-4 font-medium min-w-[140px] text-right">Hành động</th>
                </tr>
              </thead>
                ...
                {filteredRegistrations.length === 0 ? (
                  <tr>
                    <td colSpan="8" className="px-6 py-16 text-center text-gray-500">
                      Không tìm thấy biểu mẫu đăng ký nào khớp với tiêu chí tìm kiếm.
                    </td>
                  </tr>
                ) : (
                  filteredRegistrations.map((reg, idx) => {
                    ...
                    return (
                      <tr key={reg._id} className="hover:bg-gray-800/20 transition-colors">
                        <td className="px-6 py-4 text-center text-gray-500 font-medium text-xs">
                          {idx + 1}
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-white font-medium">{reg.parentName}</p>
                        </td>
                        <td className="px-6 py-4 space-y-1">
                          ...
                          {reg.email && (
                            <p className="text-gray-400 text-xs font-mono">{reg.email}</p>
                          )}
                        </td>
                        <td className="px-6 py-4">
                          <p className="text-white">{reg.childName}</p>
                          {reg.childAge && (
                            <p className="text-xs text-gray-500 mt-0.5">{reg.childAge} tuổi</p>
                          )}
                        </td>
                        <td className="px-6 py-4 max-w-[250px] whitespace-normal text-xs text-gray-400">
                          {reg.notes ? (
                            <div className="flex items-start gap-1">
                              <Note size={14} className="text-gray-500 mt-0.5 shrink-0" />
                              <span>{reg.notes}</span>
                            </div>
                          ) : (
                            <span className="text-gray-600 italic">Không có ghi chú</span>
                          )}
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2.5 py-1 rounded-full text-xs font-medium inline-block ${statusInfo.class}`}>
                            {statusInfo.text}
                          </span>
                        </td>
