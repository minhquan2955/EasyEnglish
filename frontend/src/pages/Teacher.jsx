...
import { ChalkboardTeacher, Plus, IdentificationBadge, EnvelopeSimple, Phone, LockKey, PencilSimple, Tag, Money, Clock } from '@phosphor-icons/react';
...
export default function Teacher() {
  ...
  const handleEdit = (teacher) => {
    ...
    setFormData({
      ...
      specializations: teacher.specializations ? teacher.specializations.join(', ') : '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        weeklySessionLimit: formData.weeklySessionLimit ? Number(formData.weeklySessionLimit) : undefined,
        salary: {
      ...
      if (!response.ok) {
        throw new Error(data.message || (editingId ? 'Lỗi khi cập nhật giáo viên' : 'Lỗi khi tạo giáo viên'));
      }

      setSuccess(editingId ? `Cập nhật giáo viên thành công!` : `Tạo giáo viên thành công!`);
    ...
    <div className="p-8 max-w-7xl mx-auto text-white">
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400">Xem danh sách giáo viên và hồ sơ công tác.</p>
        </div>
      ...
      {activeTab === 'list' && (
        <div className="bg-[#121314] rounded-[8px] border border-gray-800 p-6">
          <div className="flex items-center gap-2 mb-6">
            ...
            <h2 className="text-xl" style={{ fontWeight: 300 }}>Danh sách Giáo viên</h2>
          </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {teachers.length === 0 ? (
                    ...
                    teachers.map((tc) => (
                      <tr key={tc._id} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                        <td className="py-4 text-white font-medium">{tc.employeeCode}</td>
                        <td className="py-4 text-gray-300">{tc.userId?.fullName || '-'}</td>
                        <td className="py-4 text-gray-400">{tc.userId?.email || '-'}</td>
                        <td className="py-4 text-gray-400">{tc.specializations?.join(', ') || '-'}</td>
                        <td className="py-4">
                          <span className={`px-2 py-1 rounded-[4px] text-xs font-medium ${
                            tc.status === 'active' ? 'bg-[#00a854]/10 text-[#00a854]' :
                            tc.status === 'on_leave' ? 'bg-yellow-500/10 text-yellow-500' : 'bg-[#c81b3a]/10 text-[#c81b3a]'
                          }`}>
                            {tc.status === 'active' ? 'Đang dạy' : tc.status === 'on_leave' ? 'Nghỉ phép' : 'Đã nghỉ'}
                          </span>
                        </td>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                {editingId && (
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-1">Trạng thái công tác</label>
                    <div className="relative">
            ...
            <div className="space-y-4 pt-4">
              <h3 className="text-lg font-medium border-b border-gray-800 pb-2">Hồ sơ giáo viên</h3>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Mã Giáo viên</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Chuyên môn (Cách nhau bằng dấu phẩy)</label>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Giới hạn tiết học (Tuần)</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-500"><Clock size={20} /></div>
                    <input type="number" name="weeklySessionLimit" value={formData.weeklySessionLimit} onChange={handleChange}
                      className="w-full h-12 pl-10 pr-4 bg-black border border-gray-800 rounded-[4px] text-white placeholder:text-gray-600 focus:outline-none focus:border-ps-blue focus:ring-1 focus:ring-ps-blue transition-colors"
                      placeholder="VD: 20" />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Loại lương</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-500"><Money size={20} /></div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Mức lương (VNĐ)</label>
                  <input type="number" name="salaryAmount" value={formData.salaryAmount} onChange={handleChange}
                    className="w-full h-12 px-4 bg-black border border-gray-800 rounded-[4px] text-white placeholder:text-gray-600 focus:outline-none focus:border-ps-blue focus:ring-1 focus:ring-ps-blue transition-colors"
