    ...
    <div className="p-8 max-w-7xl mx-auto text-white">
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400">Xem và chỉnh sửa tài khoản người dùng.</p>
        </div>
        ...
        <div className="bg-[#121314] rounded-[8px] border border-gray-800 p-6">
          <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl" style={{ fontWeight: 300 }}>Danh sách ({filteredUsers.length})</h2>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-sm text-gray-400">Lọc vai trò:</span>
              <select
                value={filterRole}
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    filteredUsers.map((user) => (
                      <tr key={user._id} className="border-b border-gray-800/50 hover:bg-gray-800/20">
                        <td className="py-4 text-white font-medium">{user.fullName}</td>
                        ...
                        <td className="py-4">
                          <span className={`px-2 py-1 rounded-[4px] text-xs font-medium ${
                            ...
                            user.role === 'teacher' ? 'bg-purple-900/30 text-purple-400' :
                            user.role === 'student' ? 'bg-blue-900/30 text-blue-400' :
                        ...
                        <td className="py-4">
                          <span className={`px-2 py-1 rounded-[4px] text-xs font-medium ${
                            user.status === 'active' ? 'bg-[#00a854]/10 text-[#00a854]' : 'bg-gray-800 text-gray-400'
                          }`}>
                            {user.status}
                          </span>
                        </td>
      ...
      {activeTab === 'edit' && (
        <div className="bg-[#121314] rounded-[8px] border border-gray-800 p-8 max-w-2xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-1">Địa chỉ Email</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-500">
              ...
              <p className="mt-1 text-xs text-gray-500">Không thể sửa email sau khi tạo.</p>
            </div>
